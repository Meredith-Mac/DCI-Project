...
Matrilines$Weaning.Date<-ymd(Matrilines$Weaning.Date)
...
# NOTE: some DBIs negative because next calf is born before
...
#add season (y/n) for first calf birth ("in season" September through February)
InSeason<-MortAccurate %>%
  mutate(DeathMonth = month(Death.Date)) %>%
  mutate(Season = ifelse(DeathMonth == 9 | DeathMonth == 10 | DeathMonth == 11 | DeathMonth == 12 |
                           DeathMonth == 1 | DeathMonth == 2, 1,0))
...
#177 intervals with new approach-- 108 moms (two year gap 173 intervals, 105 moms)
Sample<-InSeason %>%
...
#remove calves w/ age death >2yrs... 166 intervals
...
Summary<-CalfDieBefore2%>%
filter(EventCensored == 1) %>%
  group_by(CalfAgeDeathCategories) %>%
  summarise(mean.T = (mean(TimeToEvent)*12), mean.yrs = (mean(TimeToEvent)), sd.T = (sd(TimeToEvent)*12), sd.yrs = (sd(TimeToEvent)), n.Age.Cats = n())

print(Summary)

Newborn<-CalfDieBefore2%>%
  filter(EventCensored == 1) %>%
